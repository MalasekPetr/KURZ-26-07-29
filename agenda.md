...
**5 dní · zakázkový běh (revize 2026-07-28).** Dny 1–3 = API a skriptování pro
začátečníky (evangelizace M365, výběr z GOC223 + nové bloky), dny 4–5 = AI a Copilot
(výběr z GOC224). P = povinný, V = volitelný. Cílovka D1–3: firma s velkým potenciálem,
která se M365 zatím bojí využívat naplno — cíl je odbourat strach ze skriptování a API,
ne vychovat migrační inženýry.
...
| 2 | Mapa API M365 & SPO + historie nástrojů | `day-1/api-landscape` | P |
| 3 | JSON, YAML a PowerShell od nuly *(Lab 1: první skript s Copilot Chat)* | `day-1/formats-fundamentals` | P |
| 4 | VS Code + Copilot Chat workflow | `day-1/vscode-copilot-env` | P |

> [!NOTE]
> Nejrizikovější blok dne je onboarding s MFA (25 účtů, nepředvídatelný čas).
> Evangelizační tón od první minuty: `api-landscape` odpovídá na „proč se toho nebát"
> (data zůstávají v tenantu), `formats-fundamentals` dá každému účastníkovi první
> vlastnoručně otestovaný skript ještě v D1. Elastický ventil = lab bloku 4
> (workflow s Copilot Chat) — při skluzu se dokončí ráno D2.
...
> YubiKey demo je instruktorské (fyzické klíče instruktora), studenti hands-on jen
> softwarové certy. Hardening je zkrácená forma původního D3 bloku — audit toho,
> co v D2 vzniklo.
...
| 3 | Node.js & TypeScript — druhá cesta k API *(instruktorské demo)* | `day-3/node-typescript` | P |
...
> vlastní skript nad svým pracovním webem — tj. přesně workflow, který si má odnést
> do firmy. Node/TS demo nemá lab — cíl je vědět, že cesta existuje, ne ji ovládat.
